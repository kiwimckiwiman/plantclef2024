...
# Model
MLP_PATH = r"C:\Users\User\plantclef\code\model\models4\mlp_run_6_0.05_100_single_class.pth"
...
path = r"D:\plantclef2024\patch_64\CBN-can-A1-20230705.jpg"
...
for i in tqdm(os.listdir(path)):
    ...
    predictions = vit_to_mlp(img, vit, mlp)
    scores = []
    for pair in predictions:
        scores.append(pair[1])
    preds.append([i, predictions, variance(predictions)])
    
fields = ['img', 'raw_preds', 'variance']
file_name = "raw.csv"
...
def plsplsplsplspls(process):
    ...
    for image in patch_batch[int(process)]:
        ...
        print("====================================")
        print(f" MLP 1 BMA {process}: Now predicting {image} | {count} of {1695/5} | 64")
        for patch in os.listdir(folder64):
            img_path = os.path.join(folder64, patch)
            predictions = vit_to_mlp(img_path, vit, mlp)
            scores = []
            for pair in predictions:
        ...
        print("====================================")
        print(f" MLP 1 BMA {process}: Now predicting {image} | {count} of {1695/5} | 16")
    ...
    file_name = "mlp_run_1_0.05_100_single_class_64_16_BMA_z_score" + str(process) + ".csv"
    OUTPUT = r"C:\Users\User\plantclef\code\benchmark"
