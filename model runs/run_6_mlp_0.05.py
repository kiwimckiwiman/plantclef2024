...
def fit(epochs, batch_size, lr, model, opt_func = torch.optim.SGD):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch [{epoch}] of [{epochs}]| val_loss: {round((val_loss_batch/len(test_batch)),2)} top_n: {round((val_acc_top_n_batch/len(train_batch)),2)} top_10: {round((val_acc_top_10_batch/len(train_batch)),2)} tp_acc: {round((val_acc_tp_acc_batch/len(train_batch)),2)}")
        ...
        #save
        if(epoch+1 == 100):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.05_100.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 150):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.05_150.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 200):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.05_200.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 250):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.05_250.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 300):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.05_300.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 350):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.05_350.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 400):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.05_400.pth"
            torch.save(mlp.state_dict(), PATH)
fit(400, 128, 0.05, mlp)