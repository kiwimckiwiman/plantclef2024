...
MODEL = r"C:\Users\User\plantclef\code\model\models\mlp_run_8_0.07_merged_sig_100_single_class.pth"
...
def get_train():
    dataset = []
    selected = random.sample(train_list, 7680)
    for i in tqdm(selected):
        embed = np.load(os.path.join(TRAIN, i))
        one_hot = []
        for class_name in CLASSES:
            item_labels = i.split(".")[0].split("_")[:-1]
...
def get_test():
    dataset = []
    selected = random.sample(test_list, 7680)
    for i in tqdm(selected):
        embed = np.load(os.path.join(TEST, i))
        one_hot = []
        for class_name in CLASSES:
            item_labels = i.split(".")[0].split("_")[:-1]
            if class_name in item_labels:
...
def fit(epochs, batch_size, lr, model, opt_func = torch.optim.SGD):
    ...
    for epoch in range(epochs):
        ...
        #save
        if(epoch+1 == 100):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_8_0.07_merged_sig_100.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 150):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_8_0.07_merged_sig_150.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 200):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_8_0.07_merged_sig_200.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 250):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_8_0.07_merged_sig_250.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 300):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_8_0.07_merged_sig_300.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 350):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_8_0.07_merged_sig_350.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 400):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_8_0.07_merged_sig_400.pth"
            torch.save(mlp.state_dict(), PATH)
