...
def fit(epochs, batch_size, lr, model, opt_func = torch.optim.SGD):
    ...
    for epoch in range(epochs):
        ...
        #save
        if(epoch+1 == 100):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.1_200.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 150):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.1_250.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 200):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.1_300.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 250):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.1_350.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 300):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.1_400.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 350):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.1_450.pth"
            torch.save(mlp.state_dict(), PATH)
        if(epoch+1 == 400):
            PATH = r"C:\Users\User\plantclef\code\model\mlp_run_6_0.1_500.pth"
            torch.save(mlp.state_dict(), PATH)
fit(400, 128, 0.1, mlp)