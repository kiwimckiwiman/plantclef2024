...
def main():
    ...
    for i in range(0, num_processes):
        if os.name == 'nt':  # Windows
            ...
            command = f'cmd.exe /k "cd /d {script_directory} && {venv_activate} && python godpleasework2.py {i}"'
            process = subprocess.Popen(command, shell=True)
        else:  # Unix-based systems
            # Command to activate the virtual environment and run the script
            command = f'bash -c "cd {script_directory} && source {venv_activate} && python godpleasework2.py {i}"'
