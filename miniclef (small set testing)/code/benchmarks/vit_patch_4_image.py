...
RESULTS = r"C:\Users\ASUS\Desktop\miniclef\code\benchmarks\results\vit_patch_4_image_top_50.csv"
...
top_n = 50
all_pred = []
for name in tqdm(os.listdir(TEST_IMGS)):
    img = Image.open(os.path.join(TEST_IMGS, name))
    plt.imshow(img)
    plt.show()
    width, height = img.size
    patches = patchify(np.array(img), [round(height/2),round(width/2),3], step = round(width/2))
    all_patch_pred = []
    for i in range(patches.shape[0]):
      for j in range(patches.shape[1]):
          patch = patches[i, j, 0]
          patch = Image.fromarray(patch)
          output = model(transforms(patch).unsqueeze(0).to(device))
          ...
          all_patch_pred.append(predictions)
    all_pred.append([name, all_patch_pred])
