...
TEST = r"D:\plantclef2024\patch_64"
...
RES = r"C:\Users\User\plantclef\code\benchmark\64"
#%% Load DinoV2
...
def get_pred(img):
    ...
    predictions = []
    for i in range(100):
        predictions.append([pred_labels[i], score[i]])
    
    #calculate variance of set
    return variance(score), predictions
...
for folder in os.listdir(TEST):
    folder = 'CBN-PdlC-A6-20130807.jpg'
    print(f"Now predicting {folder} | {count} of {len(os.listdir(TEST))}")
    count += 1
    ...
    print(f"Predicting {N} patches...")
    img_predictions = []
    img_res = folder.split(".")[0] + "_raw.csv"
    for img_name in tqdm(os.listdir(folder_path)):
        ...
        img_predictions.append([preds, var])
        # all_preds.append([preds, numer])
        # for i in preds: # add seen classes into list for class-wise BMA
        #     if i[0] not in classes:
        #         classes.append(i[0])
        
    fields = ['top_100', 'variance']
    with open(os.path.join(RES, img_res), 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(fields)
        csvwriter.writerows(img_predictions)
    BMA_scores = []
    break
    # print("Calculating BMA...")
    # for c in tqdm(classes):
    #     BMA = 0 #initialise
    #     for pred in all_preds: #foreach patch 
    #         score= pred[0]
    #         numer = pred[1]
    #         for label, score in score:
    #             if label == c: #if prediction class matches current class iter
    #                 BMA += score * (numer/BMA_denom) #calculate BMA for pred for class c
...
#%%
# fields = ['img_name', 'top_50_BMA_unsorted']
