...
for file in os.listdir(TRAIN):
    ...
    if processed:
        ...
        try:
            ...
            random_100_avg = []
            #random 100 avg
            if(len(real_embeddings) >= 100):
                print(file + ": calculating random 100 average")
                random_100 = random.sample(real_embeddings, 100)
                for e in tqdm.tqdm(random_100):
                    if len(random_100_avg) == 0:
                        random_100_avg = torch.from_numpy(e)
                    else:
                        random_100_avg = torch.add(random_100_avg, torch.from_numpy(e))
                random_100_avg = random_100_avg.apply_(lambda x: (x/len(random_100_avg)))
